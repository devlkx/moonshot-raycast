...
export function useModel(): ModelHook {
  ...
  const [option, setOption] = useState<Model["option"][]>(["moonshot-v1-8k", "moonshot-v1-32k", "moonshot-v1-128k"]);
  ...
  useEffect(() => {
    gpt.models
      ...
      .then((res) => {
        ...
        setOption(models.filter((m) => m.id.startsWith("moonshot")).map((x) => x.id));
      })
      .catch(async (err) => {
        console.error(err);
